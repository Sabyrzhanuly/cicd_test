...
Тестовый sandbox: **каждый пушит в `dev/<имя>`**, PR — в защищённые ветки.

**Компания:** `develop` → `stage` → `master` (три отдельных PR из `dev/*`, без merge между ними).  
**Sandbox:** `develop` + `main` (`main` = `master`, `stage` скоро).
...
| Ветка | Push | PR |
|-------|------|-----|
| `dev/<имя>` | ✅ push | → `develop`, → `stage`*, → `master` |
| `develop` | ❌ | PR из `dev/*` |
| `stage` | ❌ | PR из `dev/*` *(скоро)* |
| `master` / `main`* | ❌ | PR из `dev/*` |
...
Ветки **независимы**: только `dev/* → <среда>`, **не** `develop → stage → master`.

Отчёт: [Issue #9](https://github.com/Sabyrzhanuly/cicd_test/issues/9) · [BRANCH_STATUS.md](./BRANCH_STATUS.md)

**Telegram:** после merge — [как вступить в группу и что делать](./TELEGRAM.md)
